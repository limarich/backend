...
export class InMemoryBusinessModelRepository implements BusinessModelRepository{
    public businessModel: BusinessModel[] = [];
    async create(data: Prisma.BusinessModelCreateInput) {
        const {business,channels,costs,customerRelationship,customerSegments,id,mainActivities,mainPartnerships,mainResources,revenue,valueProposition} = data;
        this.businessModel.push({
            businessId: business?.connect?.id ?? 'generated-id',
            channels: Array.isArray(channels) ? channels : [],
            costs: Array.isArray(costs) ? costs : [],
            customerRelationship: Array.isArray(customerRelationship) ? customerRelationship : [],
