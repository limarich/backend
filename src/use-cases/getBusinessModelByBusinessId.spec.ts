...
import { GetBusinessModelByBusinessIdUseCase } from "./getBusinessModelByBusinessId";
...
describe("get Business Model Swot use case", () => {
  it("should be able to get data", async () => {
    ...
    const response = await getBusinessModelByBusinessIdUseCase.execute(
  ...
  it("should not be able to get data", async () => {
    ...
    await expect(() => {
      return getBusinessModelByBusinessIdUseCase.execute("123456");
